...
| E3 | Second dataset (reference doc names DVS128 Gesture as Step 2) — needs a `DATASETS` entry and probably different architecture knobs | not started |
...
| G3 | Machine-specific config (e.g. `num_workers` 0 locally vs 2 on Colab) via `extends:` — mechanism exists, config not written | open |
