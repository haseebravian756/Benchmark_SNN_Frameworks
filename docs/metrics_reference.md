...
| **spikes per neuron per inference** | spikes | The same quantity in the unit the SNN literature normally uses. One inference = one forward pass over all T timesteps for one sample. | `spike_rate_pct / 100 × T`. Printed next to the percentage; derivable from two columns already in `runs.csv`, so it is not stored separately. | Measured 2.06% at T=20 → **0.41 spikes/neuron/inference**. Directly comparable to published figures: [Nature Communications 2024](https://www.nature.com/articles/s41467-024-51110-5) reports convolutional SNNs at *"about 0.4 spikes per neuron"* and MLP-style SNNs at 0.1–0.2. |
...
stronger caveat than the usual "energy is a proxy for framework overhead"
framing, and it should replace that wording.
